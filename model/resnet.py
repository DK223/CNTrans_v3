...
class MyResnet(ResNet):
    def __init__(self, args):
        super().__init__(Bottleneck, [3, 4, 6, 3], num_classes=args.d_model)
...
class Joiner(nn.Sequential):
    ...
    def forward(self, tensor_list: Tensor):
        xs = self[0](tensor_list)  # 用backbone处理输入的图片
        out: List[Tensor] = []
        pos = []
        for x in xs:
            out.append(x)
...
def build_backbone(args):
    position_embedding = build_position_encoding(args.hidden_dim, args.src_len, args.pos_emb_type)
    # train_backbone = args.lr_backbone > 0
    # return_interm_layers = args.masks
    # backbone = Backbone(args.backbone, train_backbone, return_interm_layers, args.dilation)
    backbone = MyResnet(args)
    model = Joiner(backbone, position_embedding)
    model.num_channels = args.d_model
    return model
